...
* Defines the *entry point* as `./src/main/js/app.js`. In essence, `app.js` (a module you will write shortly) is the proverbial `public static void main()` of our JavaScript application. `webpack` must know this in reservation to know _what_ to launch when the final bundle is loaded by the browser.
...
* It hooks into the babel engine, using both `es2015` and `react` presets, in reservation to compile ES6 React code into a format able to be run in any standard browser.
